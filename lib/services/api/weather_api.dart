...
class WeatherApi {
  ...
  getFiveDayWeather(double latitude, double longitude, Function parentState) async {
    // If the forecast has not been polled, or it has not been polled in 5 minutes, poll it again
    if (fiveDayForecast == null || DateTime.now().difference(DateTime.fromMillisecondsSinceEpoch(fiveDayForecast.list[0].dt * 1000)).inMinutes >= 5) {

    }
    Uri uri = buildUri(_forecastEndpoint, latitude, longitude);

    http.Response response = await http.get(uri);
    fiveDayForecast = fiveDayForecastFromJson(response.body);
    print(fiveDayForecast.list.length);
    print(DateTime.fromMillisecondsSinceEpoch(fiveDayForecast.list[0].dt * 1000));
    print('I got a five day forecast');

    parentState();
  }

  getCurrentWeather(double latitude, double longitude, Function parentState) async {
